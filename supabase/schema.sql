...
CREATE TABLE public.profiles (
  ...
  match_same_origin BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE public.lifestyle_preferences (
  ...
  deal_breaker      TEXT NOT NULL DEFAULT 'none'
...
CREATE TABLE public.flat_swipes (
  id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE POLICY "flat_swipes_select_own" ON public.flat_swipes FOR SELECT
  USING (auth.uid() = profile_id OR EXISTS (
    SELECT 1 FROM public.matches m WHERE m.id = match_id AND (m.requester_id = auth.uid() OR m.receiver_id = auth.uid())
  ));
